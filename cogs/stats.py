...
class Stats(commands.Cog):
    ...
    def _clear_gateway_data(self):
        ...
        to_remove = [index for index, dt in enumerate(
            self._resumes) if dt < one_week_ago]
    ...
    async def show_guild_stats(self, ctx):
        ...
        embed.set_footer(
            text='Tracking command usage since').timestamp = count[1] or datetime.datetime.utcnow()
    ...
    async def show_member_stats(self, ctx, member):
        ...
        embed.description = f'{count[0]} commands used.'
        embed.set_footer(
            text='First command used').timestamp = count[1] or datetime.datetime.utcnow()
    ...
    async def bothealth(self, ctx):
        ...
        for index, holder in enumerate(pool._holders, start=1):
            ...
            questionable_connections += any((in_use,
        ...
        embed.add_field(name='Events Waiting',
                        value=f'Total: {len(event_tasks)}', inline=False)
    ...
    async def gateway(self, ctx):
        ...
        for shard_id, shard in self.bot.shards.items():
            ...
            if identify == 0:
                ...
                builder.append(
                    f'Shard ID {shard_id}: {badge} ({identify} IDENTIFYs)')
    ...
    async def command_history_log(self, ctx, days=7):
        ...
        as_data = sorted(all_commands.items(),
        ...
        embed.set_footer(text='Since').timestamp = datetime.datetime.utcnow(
        ) - datetime.timedelta(days=days)

        top_ten = '\n'.join(
            f'{command}: {uses}' for command, uses in records[:10])
        bottom_ten = '\n'.join(
            f'{command}: {uses}' for command, uses in records[-10:])
