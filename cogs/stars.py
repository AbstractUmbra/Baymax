...
class Starboard(db.Table):
    ...
    max_age = db.Column(
        db.Interval, default="'7 days'::interval", nullable=False)
...
class StarboardEntry(db.Table, table_name='starboard_entries'):
    ...
    guild_id = db.Column(db.ForeignKey(
        'starboard', 'id', sql_type=db.Integer(big=True)), index=True, nullable=False)


class Starrers(db.Table):
    id = db.PrimaryKeyColumn()
    author_id = db.Column(db.Integer(big=True), nullable=False)
    entry_id = db.Column(db.ForeignKey(
        'starboard_entries', 'id'), index=True, nullable=False)
...
class StarboardConfig:
    __slots__ = ('bot', 'id', 'channel_id', 'threshold',
                 'locked', 'needs_migration', 'max_age')
...
class Stars(commands.Cog):
    ...
    def get_emoji_message(self, message, stars):
        ...
        embed.add_field(name='Original',
                        value=f'[Jump!]({message.jump_url})', inline=False)
    ...
    async def get_message(self, channel, message_id):
        try:
            ...
            try:
                ...
                def pred(m): return m.id == message_id
    ...
    async def star_clean(self, ctx, stars=1):
        ...
        min_snowflake = int((time.time() - 14 * 24 * 60 * 60)
    ...
    async def star_who(self, ctx, message: MessageID):
        ...
        try:
            p = Pages(ctx, entries=members, per_page=20,
    ...
    async def star_migrate(self, ctx):
        ...
        async with ctx.typing():
            ...
            e = discord.Embed(title='Starboard Migration',
    ...
    async def star_guild_stats(self, ctx):
        ...
        e.add_field(name='Top Starred Posts',
                    value=self.records_to_value(starred_posts), inline=False)

        def to_mention(o): return f'<@{o}>'

        star_receivers = [r for r in records if r['Type'] == 1]
        value = self.records_to_value(
            star_receivers, to_mention, default='No one!')
        e.add_field(name='Top Star Receivers', value=value, inline=False)

        star_givers = [r for r in records if r['Type'] == 2]
        value = self.records_to_value(
            star_givers, to_mention, default='No one!')
    ...
    async def star_member_stats(self, ctx, member):
        ...
        e.add_field(name='Top Starred Posts',
                    value=self.records_to_value(top_three), inline=False)
