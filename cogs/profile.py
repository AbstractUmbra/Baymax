...
class DisambiguateMember(commands.IDConverter):
    async def convert(self, ctx, argument):
        ...
        if len(argument) > 5 and argument[-5] == '#':
            ...
            def pred(u): return u.name == name and u.discriminator == discriminator
        ...
        if result is None:
            raise commands.BadArgument(
                "Could not found this member. Note this is case sensitive.")
        return result
...
_rank = re.compile(
    r'^(?P<mode>\w+(?:\s*\w+)?)\s*(?P<rank>[AaBbCcSsXx][\+-]?)\s*(?P<number>[0-9]{0,4})$')
...
def valid_rank(argument, *, _rank=_rank):
    ...
    try:
        ...
        raise commands.BadArgument(
            f'Unknown Splatoon 2 mode: {mode}') from None
...
def valid_squad(argument):
    ...
    if len(arg) > 100:
        raise commands.BadArgument(
            'Squad name way too long. Keep it less than 100 characters.')
...
_friend_code = re.compile(
    r'^(?:(?:SW|3DS)[- _]?)?(?P<one>[0-9]{4})[- _]?(?P<two>[0-9]{4})[- _]?(?P<three>[0-9]{4})$')
...
class SplatoonWeapon(commands.Converter):
    async def convert(self, ctx, argument):
        ...
        if cog is None:
            raise commands.BadArgument(
                'Splatoon related commands seemingly disabled.')

        query = argument.strip('"')
        if len(query) < 4:
            raise commands.BadArgument(
                'Weapon name to query must be over 4 characters long.')
...
class Profile(commands.Cog):
    ...
    async def profile(self, ctx, *, member: DisambiguateMember = None):
        ...
        if rank:
            value = '\n'.join(
                f'{mode}: {data["rank"]}{data["number"]}' for mode, data in rank.items())
    ...
    async def search(self, ctx, *, query):
        ...
        e.set_footer(text=format(plural(len(records)),
                                 'record') + '\u2003' * 60 + '\u200b')
        await ctx.send(embed=e)
