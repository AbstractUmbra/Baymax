...
MUTE_RE = re.compile(
    r"([^#]+#\d{4} \(ID: (\d+)\):(.*))?(Action done by [^#]+#\d{4} \(ID: (\d+)\))?", re.IGNORECASE)
...
class Private(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.colours = {ModActions.banned: 0xfffffe,
    ...
    def gen_embed(self, event: ModActions, *, member: Union[Member, User], entry: AuditLogEntry, **kwargs):
        ...
        embed = Embed(title=f"D.py {event.name}",
                      colour=colour)
    ...
    async def on_member_ban(self, guild: Guild, person: Union[User, Member]):
        ...
        embed = self.gen_embed(
            ModActions.banned, member=person, entry=real_entry)
    ...
    async def on_member_remove(self, member: Member):
        ...
        if not real_entry:
            return
        embed = self.gen_embed(
            ModActions.kicked, member=member, entry=real_entry)
        await self.bot.get_user(self.bot.owner_id).send(embed=embed)
        if self.webhook:
            await self.webhook.send(embed=embed)
    ...
    async def on_member_unban(self, guild: Guild, person: User):
        ...
        if not real_entry:
            return
        embed = self.gen_embed(ModActions.unbanned,
                               member=person, entry=real_entry)
        await self.bot.get_user(self.bot.owner_id).send(embed=embed)
        if self.webhook:
            await self.webhook.send(embed=embed)
    ...
    async def on_guild_channel_update(self,
        ...
        new_overwrite_for = real_entry.extra if isinstance(
            real_entry.extra, Member) else None

        if not new_overwrite_for:
            # entries return no accurate search
            await self.bot.get_user(self.bot.owner_id).send(f"failed:\nEntry: {real_entry}\nMember target: {new_overwrite_for}.")
            return
        ...
        if reason_expanded:
            ...
            expires = datetime.datetime.strptime(
                reason_expanded[2], "%Y-%m-%d %H:%M:%S.%f")
            embed = self.gen_embed(
                ModActions.helpblocked, member=new_overwrite_for, entry=real_entry, helper=helper, expires=expires)
            await self.bot.get_user(self.bot.owner_id).send(embed=embed)
            if self.webhook:
                await self.webhook.send(embed=embed)
        else:
            fmt = real_entry.reason
            await self.bot.get_user(self.bot.owner_id).send(f"reason_expanded failed {fmt}")
    ...
    async def on_member_update(self, before: Member, after: Member) -> Message:
        ...
        if after._roles.has(570243756130041876) and not before._roles.has(570243756130041876):
            # Muted
            await asyncio.sleep(3)

            real_entry = None
            async for entry in after.guild.audit_logs(action=AuditLogAction.member_role_update, limit=15):
                reason = getattr(entry, "reason", "No reason")
            ...
            embed = self.gen_embed(
                ModActions.muted, member=after, entry=real_entry, mod=mod, reason=mod_reason)
            await self.bot.get_user(self.bot.owner_id).send(embed=embed)
            if self.webhook:
                await self.webhook.send(embed=embed)
    ...
    async def on_member_join(self, member):
        if member.guild.id == 766520806289178646:
            if member.bot:
                return await member.add_roles(Object(id=766522043143290900))
            else:
                return await member.add_roles(Object(id=766525464092868628))
