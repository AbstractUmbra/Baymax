...
class Context(commands.Context):
    ...
    async def safe_send(
        self, content, *, escape_mentions=True, suffix: str = None, **kwargs
    ):
        ...
        if len(content) > 2000:
            link = await self.bot.mb_client.post(
                content, syntax=f".{suffix if suffix else ''}"
            )
            return await self.send(
                f"Output too long, here it is on Mystb.in: {link}.", **kwargs
            )
        else:
